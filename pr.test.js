...
test('can extract a PR number from a PR merge commit message', () => {
    expect(extractPRNumber('Merge pull request #4 from some/mockBranch')).toEqual(
        '4',
    )
    expect(
        extractPRNumber('Merge pull request #42 from some/mockBranch'),
    ).toEqual('42')
})
...
test('can extract a PR number from a squash merge commit message', () => {
    ...
    expect(
        extractPRNumber('Squash pr (#45)\n\n multiple commit messages appended'),
    ).toEqual('45')
})

test('returns null if no PR number is found in a commit message', () => {
    expect(extractPRNumber('Merge branch master into some/mockBranch')).toEqual(
        null,
    )
})
...
test("searchPRByCommit returns a PR", async () => {
  ...
  const config = {
    ...
    expect(searchPRByCommit(sha, config)).resolves.toEqual({
        number: 15,
        id: sha,
    })
})
...
test("searchPRByCommit Fails to find PR", async () => {
  ...
  expect(searchPRByCommit(sha, config)).rejects.toThrow(
